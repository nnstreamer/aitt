...
TEST_F(MQMockTest, Construtor_P_Anytime)
{
...
TEST_F(MQMockTest, Construtor_lib_init_N_Anytime)
{
    ...
    EXPECT_THROW({ aitt::MosquittoMQ mq(TEST_CLIENT_ID, true); }, aitt::AittException);
}

TEST_F(MQMockTest, Construtor_new_N_Anytime)
{
    EXPECT_CALL(mqttMock, mosquitto_lib_init()).WillOnce(Return(MOSQ_ERR_SUCCESS));
    EXPECT_CALL(mqttMock, mosquitto_new(testing::StrEq(TEST_CLIENT_ID), true, testing::_))
...
TEST_F(MQMockTest, Construtor_option_N_Anytime)
{
    EXPECT_CALL(mqttMock, mosquitto_lib_init()).WillOnce(Return(MOSQ_ERR_SUCCESS));
    EXPECT_CALL(mqttMock, mosquitto_new(testing::StrEq(TEST_CLIENT_ID), true, testing::_))
          .WillOnce(Return(TEST_HANDLE));
    EXPECT_CALL(mqttMock, mosquitto_int_option(TEST_HANDLE, testing::_, testing::_))
...
TEST_F(MQMockTest, Connect_N_Anytime)
{
    EXPECT_CALL(mqttMock, mosquitto_lib_init()).WillOnce(Return(MOSQ_ERR_SUCCESS));
    EXPECT_CALL(mqttMock, mosquitto_new(testing::StrEq(TEST_CLIENT_ID), true, testing::_))
          .WillOnce(Return(TEST_HANDLE));
    EXPECT_CALL(mqttMock, mosquitto_message_v5_callback_set(TEST_HANDLE, testing::_)).Times(1);
    EXPECT_CALL(mqttMock, mosquitto_connect(TEST_HANDLE, testing::StrEq(TEST_HOST), TEST_PORT, 60))
          .WillOnce(Return(MOSQ_ERR_PROTOCOL));
    EXPECT_THROW(
          {
              aitt::MosquittoMQ mq(TEST_CLIENT_ID, true);
              mq.Connect(TEST_HOST, TEST_PORT, "", "");
...
TEST_F(MQMockTest, Connect_loop_N_Anytime)
{
    EXPECT_CALL(mqttMock, mosquitto_lib_init()).WillOnce(Return(MOSQ_ERR_SUCCESS));
    EXPECT_CALL(mqttMock, mosquitto_new(testing::StrEq(TEST_CLIENT_ID), true, testing::_))
          .WillOnce(Return(TEST_HANDLE));
    ...
    EXPECT_CALL(mqttMock, mosquitto_connect(TEST_HANDLE, testing::StrEq(TEST_HOST), TEST_PORT, 60))
          .WillOnce(Return(MOSQ_ERR_SUCCESS));
    EXPECT_CALL(mqttMock, mosquitto_loop_start(TEST_HANDLE)).WillOnce(Return(MOSQ_ERR_ERRNO));
    EXPECT_THROW(
          {
              aitt::MosquittoMQ mq(TEST_CLIENT_ID, true);
              mq.Connect(TEST_HOST, TEST_PORT, "", "");
...
TEST_F(MQMockTest, Connect_User_N_Anytime)
{
...
TEST_F(MQMockTest, Set_will_N_Anytime)
{
    EXPECT_CALL(mqttMock, mosquitto_lib_init()).WillOnce(Return(MOSQ_ERR_SUCCESS));
    EXPECT_CALL(mqttMock, mosquitto_new(testing::StrEq(TEST_CLIENT_ID), true, testing::_))
          .WillOnce(Return(TEST_HANDLE));
    ...
    EXPECT_THROW(
          {
              aitt::MosquittoMQ mq(TEST_CLIENT_ID, true);
              mq.SetWillInfo("lastWill", TEST_PAYLOAD, sizeof(TEST_PAYLOAD), AITT_QOS_AT_MOST_ONCE,
                    true);
          },
          aitt::AittException);
}
...
TEST_F(MQMockTest, Disconnect_N_Anytime)
{
    EXPECT_CALL(mqttMock, mosquitto_lib_init()).WillOnce(Return(MOSQ_ERR_SUCCESS));
    EXPECT_CALL(mqttMock, mosquitto_new(testing::StrEq(TEST_CLIENT_ID), true, testing::_))
          .WillOnce(Return(TEST_HANDLE));
    EXPECT_CALL(mqttMock, mosquitto_message_v5_callback_set(TEST_HANDLE, testing::_)).Times(1);
    EXPECT_CALL(mqttMock, mosquitto_disconnect(testing::_)).WillOnce(Return(MOSQ_ERR_NO_CONN));
    EXPECT_CALL(mqttMock, mosquitto_will_clear(TEST_HANDLE)).Times(0);
    EXPECT_CALL(mqttMock, mosquitto_destroy(TEST_HANDLE)).Times(1);
    EXPECT_CALL(mqttMock, mosquitto_lib_cleanup()).WillOnce(Return(MOSQ_ERR_SUCCESS));
    try {
        aitt::MosquittoMQ mq(TEST_CLIENT_ID, true);
...
TEST_F(MQMockTest, Publish_N_Anytime)
{
    EXPECT_CALL(mqttMock, mosquitto_lib_init()).WillOnce(Return(MOSQ_ERR_SUCCESS));
    EXPECT_CALL(mqttMock, mosquitto_new(testing::StrEq(TEST_CLIENT_ID), true, testing::_))
          .WillOnce(Return(TEST_HANDLE));
    ...
    EXPECT_CALL(mqttMock, mosquitto_publish(TEST_HANDLE, testing::_, testing::StrEq(TEST_TOPIC),
                                sizeof(TEST_PAYLOAD), TEST_PAYLOAD, AITT_QOS_AT_MOST_ONCE, false))
          .WillOnce(Return(MOSQ_ERR_NOT_SUPPORTED));
    EXPECT_THROW(
          {
              aitt::MosquittoMQ mq(TEST_CLIENT_ID, true);
              mq.Publish(TEST_TOPIC, TEST_PAYLOAD, sizeof(TEST_PAYLOAD));
...
TEST_F(MQMockTest, Subscribe_P_Anytime)
{
    ...
    try {
        ...
        mq.Subscribe(
              TEST_TOPIC,
              [](AittMsg *info, const void *msg, const int szmsg, const void *cbdata) -> void {},
...
TEST_F(MQMockTest, Subscribe_N_Anytime)
{
    EXPECT_CALL(mqttMock, mosquitto_lib_init()).WillOnce(Return(MOSQ_ERR_SUCCESS));
    EXPECT_CALL(mqttMock, mosquitto_new(testing::StrEq(TEST_CLIENT_ID), true, testing::_))
          .WillOnce(Return(TEST_HANDLE));
    ...
    EXPECT_THROW(
          {
              ...
              mq.Subscribe(
                    TEST_TOPIC,
                    [](AittMsg *info, const void *msg, const int szmsg,
...
TEST_F(MQMockTest, Unsubscribe_N_Anytime)
{
    EXPECT_CALL(mqttMock, mosquitto_lib_init()).WillOnce(Return(MOSQ_ERR_SUCCESS));
    EXPECT_CALL(mqttMock, mosquitto_new(testing::StrEq(TEST_CLIENT_ID), true, testing::_))
          .WillOnce(Return(TEST_HANDLE));
...
TEST_F(MQMockTest, PublishWithReply_memory_N_Anytime)
{
    std::string replyTopic = "replyTopic";

    EXPECT_CALL(mqttMock, mosquitto_lib_init()).WillOnce(Return(MOSQ_ERR_SUCCESS));
    EXPECT_CALL(mqttMock, mosquitto_new(testing::StrEq(TEST_CLIENT_ID), true, testing::_))
          .WillOnce(Return(TEST_HANDLE));
    EXPECT_CALL(mqttMock, mosquitto_message_v5_callback_set(TEST_HANDLE, testing::_)).Times(1);
    EXPECT_CALL(mqttMock, mosquitto_property_add_string(testing::_, MQTT_PROP_RESPONSE_TOPIC,
                                testing::StrEq(replyTopic)))
          .WillOnce(Return(MOSQ_ERR_NOMEM));

    EXPECT_THROW(
          {
              aitt::MosquittoMQ mq(TEST_CLIENT_ID, true);
              mq.PublishWithReply(TEST_TOPIC, TEST_PAYLOAD, sizeof(TEST_PAYLOAD),
                    AITT_QOS_AT_MOST_ONCE, false, replyTopic, "");
          },
          aitt::AittException);
}

TEST_F(MQMockTest, PublishWithReply_N_Anytime)
{
    std::string replyTopic = "replyTopic";
    std::string correlation = "CorrelationData";
    ...
    EXPECT_CALL(mqttMock, mosquitto_property_add_string(testing::_, MQTT_PROP_RESPONSE_TOPIC,
                                testing::StrEq(replyTopic)))
          .WillOnce(Return(MOSQ_ERR_SUCCESS));
    EXPECT_CALL(mqttMock, mosquitto_property_add_binary(testing::_, MQTT_PROP_CORRELATION_DATA,
                                testing::_, correlation.size()))
          .WillOnce(Return(MOSQ_ERR_SUCCESS));
    EXPECT_CALL(mqttMock,
          mosquitto_publish_v5(TEST_HANDLE, testing::_, testing::StrEq(TEST_TOPIC),
                AITT_MESSAGE_MAX, TEST_PAYLOAD, AITT_QOS_AT_MOST_ONCE, false, testing::_))
          .WillOnce(Return(MOSQ_ERR_OVERSIZE_PACKET));

    EXPECT_THROW(
          {
              aitt::MosquittoMQ mq(TEST_CLIENT_ID, true);
              mq.PublishWithReply(TEST_TOPIC, TEST_PAYLOAD, AITT_MESSAGE_MAX, AITT_QOS_AT_MOST_ONCE,
                    false, replyTopic, correlation);
          },
          aitt::AittException);
}
